...
# Import Uzbek names module (optional — graceful fallback)
try:
    from uzbek_names import normalize_name, find_similar_names, ALL_UZBEK_NAMES_SET
except ImportError:
    normalize_name = None  # type: ignore
    find_similar_names = None  # type: ignore
    ALL_UZBEK_NAMES_SET = set()  # type: ignore
...
class UIAnimations:
    ...
    def loading_message(action: str, details: str = "") -> str:
        ...
        """
        emoji = UIAnimations.STAGES.get(action, "⏳")
...
class DynamicGreetings:
    ...
    MORNING_GREETINGS = [
        ...
        "Assalomu alaykum, {name}! ☀️ Yangi kun — yangi imkoniyatlar.",
        "Xayrli tong, {name}! 🌄 Bugungi rejalaringiz uchun tayyor turaman.",
    ]

    AFTERNOON_GREETINGS = [
        "Xayrli kun, {name}! ☀️ Savol bering — javob berishga tayyorman.",
...
class Config:
    ...
    def from_env(cls) -> "Config":
        load_dotenv()
        # Only BOT_TOKEN is strictly required to start
        required = {
...
async def _fetch_all_public_sheets(sheet_id: str) -> dict:
    """
    Fetch all tabs from a publicly shared Google Sheet without any OAuth credentials.

    Strategy:
      1. Call the Sheets JSON feed (no auth) to discover all tab names.
      2. For each tab, export as CSV and parse rows.
      3. Fall back to a single-sheet CSV export if the metadata call fails.

    Returns dict[sheet_title -> list[list[str]]] — same shape as the
    credentialed path so callers need no changes.
    ...
    async with _aiohttp.ClientSession() as http:
        # --- Step 1: discover tab names via the public JSON export ---
        tab_names: list[str] = []
        try:
            feed_url = f"{base_url}/gviz/tq?tqx=out:json&sheet=0"
            # The Sheets gviz endpoint returns one tab at a time; use the HTML
            # export to get all tab names from the page <title> / sheet nav.
            # Simpler: use the export/html to grab sheet names via regex.
            ...
            async with http.get(html_url, timeout=15, allow_redirects=True) as resp:
                if resp.status == 200:
                    ...
                    import re as _re
                    # Tab buttons carry data-sheet-index attributes
                    found = _re.findall(r'<li[^>]+data-sheet-index[^>]*>.*?<span[^>]*>(.*?)</span>', html_text)
                    if found:
        ...
        if not tab_names:
            tab_names = ["Sheet1"]  # fallback — just try the default tab
                ...
                async with http.get(csv_url, timeout=20, allow_redirects=True) as resp:
                    if resp.status == 200:
                        ...
                        if rows:
                            result[tab] = rows
                            any_ok = True
                            logger.info("✅ Fetched tab '%s' from sheet %s (%d rows)", tab, sheet_id, len(rows))
                    else:
                        logger.warning("CSV export status %s for tab '%s' of sheet %s", resp.status, tab, sheet_id)
            except Exception as _te:
                logger.warning("Failed to fetch tab '%s': %s", tab, _te)

        # --- Step 3: if nothing worked, raise so callers see the error ---
        if not any_ok:
            raise RuntimeError(f"Could not read any tab from sheet {sheet_id}. "
...
def build_assistant_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="💬 Chatni davom ettirish", callback_data="chat_start")],
...
def register_handlers(dp: Dispatcher, ctx: AppContext) -> None:
    ...
    async def chat_button_handler(callback_query: CallbackQuery) -> None:
        ...
        try:
            session.step = "in_chat"
            session.web_search_mode = False  # Reset web search mode when entering chat
    ...
    async def voice_hint_handler(callback_query: CallbackQuery) -> None:
        """Handle Voice Question button click — show instructions"""
        telegram_id = callback_query.from_user.id
        session = ctx.sessions.get(telegram_id)

        logger.info(f"🎤 Voice hint clicked - User {telegram_id}")

        try:
            session.step = "in_chat"
            await callback_query.message.answer(
                "🎤 <b>Ovozli savol yuboring!</b>\n\n"
                "Mikrofon tugmasini <b>bosib ushlab turing</b>, savolingizni ayting va "
                "qo'yib yuboring.",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="❌ Yopish", callback_data="chat_continue")]
                ])
            )
            await callback_query.answer()
        except Exception as exc:
            logger.exception(f"❌ Voice hint error: {exc}")
            await callback_query.answer("❌ Xatolik yuz berdi!", show_alert=True)

    @dp.message(F.voice)
    async def voice_message_handler(message: Message, bot: Bot) -> None:
        """Handle voice messages — transcribe via Whisper and answer from spreadsheet data"""
        telegram_id = message.from_user.id
        ...
        # If user is in registration flow, reject voice
        if session.step in {"waiting_first_name", "waiting_contact", "waiting_email", "waiting_last_name"}:
            await message.answer("❌ Avval ro'yxatdan o'tishni yakunlang. /start buyrug'ini yuboring.")
            return

        # Auto-enter chat mode if data is available
        if session.step not in ("in_chat",):
        ...
        if not openai_api_key:
            await message.answer(
                "❌ Ovozli savol hozir ishlamaydi (API kalit sozlanmagan).\n"
        ...
        try:
            ...
            if not ogg_bytes:
                await message.answer("❌ Ovozli xabarni yuklashda xatolik.")
                return

            # Build vocabulary from loaded sheet data for better transcription
            try:
                from audio_processor import transcribe_audio, build_sheet_vocabulary
                sheet_vocab = build_sheet_vocabulary(session)
            except ImportError:
                from audio_processor import transcribe_audio
                sheet_vocab = ""

            # Transcribe voice
            logger.info(f"🎤 Transcribing voice for user {telegram_id} ({len(ogg_bytes)} bytes)")
            ...
            if not text:
                ...
                await message.answer(
                    error_messages.get(error_key, "❌ Ovozni aniqlashda xatolik."),
                    reply_markup=build_chat_response_keyboard()
                )
                return

            logger.info(f"✅ Voice transcription for user {telegram_id}: {text[:80]}")

            # Show what was heard
            await message.answer(f"🎤 <i>Aniqlandi:</i> {text}", parse_mode="HTML")

            # Now process the transcription as a regular chat message.
            # Build a fake-ish text message to reuse the in_chat logic:
            # We replicate the core spreadsheet Q&A logic here.

            user_message = text.strip()
            if not user_message:
                await message.answer("❌ Bo'sh savol. Iltimos, qayta urinib ko'ring.")
                return
            ...
            # Check if we have local spreadsheet data
            local_context = None
            ...
            if local_context:
                # Build context text
                context_text = ""
                for sheet_id_key, sheets in local_context.items():
                    sheet_name = next(
                        (s['name'] for s in session.folder_spreadsheets if s['id'] == sheet_id_key),
                        sheet_id_key
                ...
                if grok_api_key:
                    system_prompt = (
                        ...
                        "2. Javobni ISHONCHLI va ANIQ ber.\n"
                        "3. Raqamlarni to'g'ri formatlash: 2500000 -> 2,500,000\n"
                        "4. Agar ma'lumot spreadsheetda TOPILMASA: 'Bu ma\\'lumot jadvalda mavjud emas.'\n"
                        "5. Internetdan hech qanday ma'lumot QO'SHMA.\n"
                        "6. Javobni o'zbek tilida ber.\n"
                        "7. ISMLARNI AQLLI QIDIRISH:\n"
                        "   a) -bek, -boy, -jon qo'shimchalari tushirilishi mumkin: Yodgorbek=Yodgor\n"
                        "   b) Ovozli xato: a↔o, i↔e, u↔o. Masalan: Yadgar→Yodgor\n"
                        "   c) Birinchi 3-4 harf mos kelsa — BIR XILL ODAM deb hisoblash.\n"
                    )
                    user_prompt = (
                        f"Spreadsheet ma'lumotlari:\n\n{context_text}\n\n"
                        f"Savol: {user_message}\n\n"
                        f"Spreadsheet asosida aniq javob ber."
                    )
                    ...
                    if ai_answer:
                        ...
                        response_text = f"🤖 AI Javob\n\n⚠️ AI xizmatida vaqtinchalik xatolik.\nMa'lumotlar:\n{context_text[:2000]}"
                else:
                    response_text = f"🤖 AI Javob\n\n⚠️ AI kaliti sozlanmagan.\nMa'lumotlar:\n{context_text[:2000]}"
    ...
    async def text_handler(message: Message) -> None:
        ...
        if session.step == "waiting_sheet_link":
            ...
            if sheet_id:
                try:
                    ...
                    # ── Fetch all sheets (public access, no OAuth needed) ──
                    ...
                    if all_sheets_data:
                        ...
                        # Persist workspace
                        try:
                            cid = _workspace_store.save_workspace(
                                telegram_id,
                                mode="sheets",
                                sheet_id=sheet_id,
                                sheet_name=sheet_name,
                            ...
                            if cid and cid > 0:
                                for _sn, _rows in all_sheets_data.items():
                                    _workspace_store.save_cache(cid, f"{sheet_id}::{_sn}", _sn, _rows, telegram_id=telegram_id)
                        except Exception as _ws_err:
                            logger.debug(f"Workspace save error: {_ws_err}")
                        ...
                        await message.answer(
                            ...
                            "Sabablari:\n"
                            "• Sheet umumiy (public) emas\n"
                            "• Link noto'g'ri\n\n"
                            "💡 Yechim:\n"
                            "1. Google Sheets-da \"Ulashish\" (Share) tugmasini bosing\n"
                            "2. \"Havolani bilgan har kim\" (Anyone with the link) tanlang\n"
                            "3. \"Ko'ruvchi\" (Viewer) ruxsatini bering\n"
                            "4. Havolani qayta yuboring",
        ...
        # If user has spreadsheet/excel data loaded and sends a text message
        # while in 'ready' state, automatically enter chat mode
        ...
        if session.step == "in_chat":
            ...
            try:
                        ...
                        if grok_api_key:
                            try:
                                ...
                                system_prompt = (
                                    ...
                                    "9. ISMLARNI AQLLI QIDIRISH — BU JUDA MUHIM:\n"
                                    "   a) O'zbek ismlarida -bek, -boy, -jon, -ali, -xon qo'shimchalari tushirilishi mumkin: Yodgorbek=Yodgor, Jasurbek=Jasur.\n"
                                    "   b) O'zbek tilida ovozli xato yoki talaffuz farqi tufayli harflar o'zgarishi mumkin: a↔o, i↔e, u↔o.\n"
                                    "      Masalan: 'Yadgar' so'ralganda jadvalda 'Yodgor' bo'lishi mumkin (Yadg≈Yodg), "
                                    "'Yadgar Bek' = 'Yodgorbek', 'Sardor' = 'Sardar', 'Dilnor' = 'Dilnara'.\n"
                                    "   c) Birinchi 3-4 harf mos kelsa VA ism uzunligi o'xshash bo'lsa — BIR XILL ODAM deb hisoblash.\n"
                                    "   d) Kichik/katta harf farqi bo'lmasin, transliteratsiya (lotin/kirill) ham hisobga olinsin.\n"
                                    "   e) ALGORITM: 1) Aynan moslikni tekshir. 2) Suffix qo'shib/olib moslikni tekshir. "
                                    "3) Birinchi 3-4 harf mos keladigan ismni tekshir. 4) Shundan keyingina 'topilmadi' de.\n"
                                ...
                                user_prompt = (
                                    ...
                                    f"MUHIM: Agar savoldagi ism jadvalda aynan topilmasa — "
                                    f"birinchi 3-4 harfi mos keladigan ismni ham tekshir (chunki ovozli xato bo'lishi mumkin). "
                                    f"Masalan 'Yadgar'→'Yodgor', 'Sardor'→'Sardar', 'Dilnor'→'Dilnara'. "
                                    f"Shunday ism topilsa, u haqida javob ber va '(ehtimol [aytilgan ism] nazarda tutilgan)' deb izoh qo'sh."
                                )
                                ...
                                if ai_answer:
                                    # If Grok explicitly says the data is not found, try a deterministic
                                    # fallback search in the local spreadsheet context using our
                                    # Uzbek name normaliser before returning a 'not found' answer.
                                    response_text = f"🤖 AI Javob\n\n{ai_answer}"

                                    try:
                                        if "mavjud emas" in ai_answer.lower() and local_context and normalize_name:
                                            import re

                                            # Try to extract a candidate name from the user's message
                                            candidate_name = None
                                            for tok in re.findall(r"[\w'`ʼ’\-]+", user_message, flags=re.UNICODE):
                                                norm = normalize_name(tok)
                                                if norm:
                                                    candidate_name = norm
                                                    break

                                            # If we found a candidate, scan the local_context rows
                                            if candidate_name:
                                                logger.info(f"🔎 Grok said not found — fallback deterministic search for: {candidate_name}")
                                                found_rows = []
                                                for sheet_id, sheets in local_context.items():
                                                    sheet_name = next(
                                                        (s['name'] for s in session.folder_spreadsheets if s['id'] == sheet_id),
                                                    ...
                                                    for sheet_title, rows in sheets.items():
                                                        for i, row in enumerate(rows[:MAX_ROWS_FOR_CONTEXT]):
                                                            # Normalize each cell and compare
                                                            for cell in row:
                                                                if cell is None:
                                                                    continue
                                                                ...
                                                                if not cell_s:
                                                                    continue
                                                                cell_norm = normalize_name(cell_s)
                                                                if cell_norm and cell_norm.lower() == candidate_name.lower():
                                                                    # Record matching location and row
                                                                    cells = [str(x).strip() for x in row[:MAX_COLS_FOR_CONTEXT]]
                                                                    found_rows.append((sheet_name, sheet_title, i + 1, cells))
                                                                    break

                                                if found_rows:
                                                    # Build short deterministic reply showing matches
                                                    parts = [f"🔎 Topildi (deterministik qidiruv): {candidate_name}\n"]
                                                    for sn, st, rnum, cells in found_rows[:5]:
                                                        parts.append(f"Spreadsheet: {sn}, Sheet: {st}, Row: {rnum}\n")
                                                        parts.append(" | ".join(cells) + "\n\n")

                                                    parts.append(f"(ehtimol {candidate_name} nazarda tutilgan)")
                                                    response_text = "".join(parts)
                                    except Exception as _fallback_err:
                                        logger.debug(f"🔎 Fallback search failed: {_fallback_err}")
                                else:
                ...
                # No local data and web search not enabled — tell user to load data
                if not session.web_search_mode:
                    logger.info(f"⚠️ No local spreadsheet data and web_search_mode is OFF for user {telegram_id}")
                    await message.answer(
                        "❌ Hozir faol jadval topilmadi.\n\n"
                        "📊 Jadval yoki Excel faylni yuklang:\n"
